...
<nav class="flex mb-2" aria-label="Breadcrumb"> <ol class="inline-flex items-center space-x-1 md:space-x-2">
        <li class="inline-flex items-center">
            <a href="{{ route('admin.dashboard') }}" class="text-gray-400 hover:text-primary transition-colors">
        ...
        @foreach($links as $label => $url)
            <li>
                <div class="flex items-center">
                    ...
                    @if($loop->last && $url == '#')
                        <span class="ml-1 text-xs font-semibold text-gray-500 md:ml-2">{{ $label }}</span>
                    @else
                        <a href="{{ $url }}" class="ml-1 text-xs font-medium text-gray-400 hover:text-primary transition-colors md:ml-2">{{ $label }}</a>
                    @endif
                </div>
            </li>
        @endforeach
